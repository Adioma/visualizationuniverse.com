d3.layout.orbit = function() {
	...
	_orbitLayout.start = function() {
		...
		tickInterval = setInterval(
			...
			flattenedNodes.forEach(function(_node){
				if (_node.parent) {
					var radius = _node.orbitLength ? _node.orbitLength : _node.parent.ring / 2 ; // _node.parent.ring / 2
					_node.x = _node.parent.x + ( (radius) * Math.sin( _node.angle + (currentTickStep * tickRadianStep * tickRadianFunction(_node))) );
					_node.y = _node.parent.y + ( (radius) * Math.cos( _node.angle + (currentTickStep * tickRadianStep * tickRadianFunction(_node))) );
				}
			})
	...
	//If you have an array of elements, then create a root node (center)
		...
		//otherwise assume it is an object with a root node
			...
			orbitNodes.ring = 10;//orbitSize[0] / 2;
			orbitNodes.depth = 0;
		...
		function traverseNestedData(_node) {
			...
			if(childrenAccessor(_node)) {
				...
				for (var x = 0; x<childrenAccessor(_node).length;x++) {
					...
					if(x%3 == 1) {
						childrenAccessor(_node)[x].angle = childrenAccessor(_node)[x].angle - (childrenAccessor(_node)[x].angle = childrenAccessor(_node)[x].angle/3);	
					} 
